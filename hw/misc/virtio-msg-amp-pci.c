    ...
    if (addr == A_NOTIFY) {
        if (val != 1) {
            qemu_log_mask(LOG_GUEST_ERROR,
                          "%s: unsupported notify value 0x%" PRIx64 "\n",
                          __func__, val);
            return;
        }

        vmsg_bus_amp_pci_process(VIRTIO_MSG_BUS_DEVICE(&s->fifo.dev));
    } else {
        qemu_log_mask(LOG_GUEST_ERROR,
                      "%s: write to invalid reg 0x%" HWADDR_PRIx "\n",
                      __func__, addr);
    }
}
...
static void vmsg_create_bus(VmsgAmpPciState *s)
{
    DeviceState *dev = DEVICE(s);
    Object *o = OBJECT(s);
    struct fifo_bus *fifo = &s->fifo;
    ...
    msix_vector_use(PCI_DEVICE(s), 0);
...
static void vmsg_amp_pci_realizefn(PCIDevice *dev, Error **errp)
{
    ...
    memory_region_init_ram(&s->mr_ram, OBJECT(s), "ram", 16 * KiB,
                           &error_fatal);
    ...
    msix_init_exclusive_bar(PCI_DEVICE(s), 1, 2, &error_fatal);
    vmsg_create_bus(s);
}
...
static int vmsg_amp_pci_post_load(void *opaque, int version_id)
{
    ...
    memset(&s->fifo.dev.shm, 0, sizeof(s->fifo.dev.shm));
...
static const VMStateDescription vmstate_vmsg_pci = {
    ...
    .post_load = vmsg_amp_pci_post_load,
...
static int vmsg_bus_amp_pci_send(VirtIOMSGBusDevice *bd, VirtIOMSG *msg_req)
{
    ...
    sent = spsc_send(&s->shm.device, msg_req, sizeof *msg_req);
    if (!sent) {
        vmsg_bus_amp_pci_unmap_fifo(s);
    ...
    /* Notify.  */
    msix_notify(PCI_DEVICE(pci_s), 0);
